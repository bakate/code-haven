...
import { coursesTableReducer, initialCourseTableState } from "./courses-table/reducer";
...
export const CoursesTable = () => {
  ...
  const [state, dispatch] = useReducer(coursesTableReducer, initialCourseTableState)
  ...
  const filteredItems = useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredCourses = filteredCourses.filter((course) =>
        course.titles.find(t => t.lang === locale)?.title?.toLowerCase().includes(state.filterValue.toLowerCase())
      );
    }
    if (
      ...
      filteredCourses = filteredCourses.filter((course) =>
        Array.from(state.statusFilter).includes(course.isPublished ? "published" : "unpublished")
      );
    }

    return filteredCourses;
  }, [allCourses, locale, hasSearchFilter, state.statusFilter, state.filterValue, statusOptions.length]);
  ...
  const handleSelectionChange = useCallback((keys: 'all' | Set<Key>) => {
    if (keys === 'all') {
      dispatch({ type: 'SET_SELECTED_KEYS', payload: new Set<string>(items.map(item => item.id.toString())) });
    } else {
      dispatch({ type: 'SET_SELECTED_KEYS', payload: new Set<string>(Array.from(keys).map(key => key.toString())) });
    }
  }, [items]);
  ...
  return (
    <Table
      ...
      isHeaderSticky
      bottomContent={<TableBottomContent dispatch={dispatch} pages={pages} state={state} totalItems={filteredItems.length} />}
      ...
      topContent={<TableTopContent state={state} dispatch={dispatch} statusOptions={statusOptions} headerColumns={headerColumns} />}
      topContentPlacement="outside"
      sortDescriptor={state.sortDescriptor}
      onSortChange={(sortDescriptor) => {
        dispatch({ type: 'SET_SORT_DESCRIPTOR', payload: sortDescriptor as StateType['sortDescriptor'] });
      }}
      ...
      <TableBody emptyContent={t('noCoursesFound')} items={sortedItems}>
        {(item) => (
          <TableRow key={item.id}>
            {(columnKey) => (
              <TableCell>{<TableRowContent course={item} allCategories={allCategories} columnKey={columnKey} />}</TableCell>
            )}
          </TableRow>
        )}
      </TableBody>
